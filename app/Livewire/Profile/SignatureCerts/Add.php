...
class Add extends Component
{
    ...
    public function submit()
    {
        ...
        $createDigitalSignature =  $this->digitalSignatureService->generate(
            user: $this->user,
            nama: $this->nama,
            org: $this->org,
        ...
        if ($createDigitalSignature['status']) {

            $this->toast()
                ->success('Berhasil', $createDigitalSignature['message'])
                ->send();

            $this->dispatch('cert-created');
            $this->dispatch('close-modal', id: 'modal-new-certificate');
        } else {
            $this->toast()
                ->error('Tidak Berhasil', $createDigitalSignature['message'])
                ->send();
        }
    }
